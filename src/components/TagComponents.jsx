...
const TagChip = ({
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 bg-blue-100 text-blue-800 rounded-full font-medium ${
        sizeClasses[size]
...
const TagInput = ({
  ...
  const removeTag = (tagToRemove) => {
    onTagsChange(
      tags.filter(
        (tag) => (tag.name || tag) !== (tagToRemove.name || tagToRemove)
      )
    );
  };
